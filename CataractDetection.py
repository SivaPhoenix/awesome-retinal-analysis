...
def maint():
    ...
    cv2.imwrite('./experiments/VesselNet/test/sema/1.jpg',img)        # Display original image
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)    # Grayscale conversion
    # cv2.imshow("1 - Grayscale Conversion", gray) 
    cv2.imwrite('./experiments/VesselNet/test/sema/2.jpg',gray)    # Display grascale image
    ...
    cv2.imwrite('./experiments/VesselNet/test/sema/3.jpg',imgfiltered)      # Display Filtered Image
    ...
    cv2.imwrite('./experiments/VesselNet/test/sema/4.jpg',thresh_image)                                   # Display image after thresholding
    ...
    cv2.imwrite('./experiments/VesselNet/test/sema/5.jpg',morpho)                                # Display image after morphological opening
    cimg_morpho = img.copy()                                                        # Copy the semaant image
    ...
    for i in circles[0,:]:
        cv2.circle(cimg_morpho,(int(i[0]),int(i[1])),int(i[2]),(0,255,0),2)        # Draw the outer circle
        cv2.circle(cimg_morpho,(int(i[0]),int(i[1])),int(2),(0,0,255),3)           # Draw the center of the circle

    # cv2.imshow("5 - Find Circles", cimg_morpho)                     # Display the image with circles detected
    img_morpho_copy = morpho.copy()                                 # Copy the image semaing from morphological opening
    ...
    # cv2.imshow("8 - Iris Contour Separation", img_morpho_copy) 
    cv2.imwrite('./experiments/VesselNet/test/sema/6.jpg',img_morpho_copy)  # Display image after morphological opening
    # cv2.imshow("7 - Image Inversion", imgg_inv)     
    cv2.imwrite('./experiments/VesselNet/test/sema/7.jpg',imgg_inv)                 # Display image after detection and separation of circle - this is our mask
    ...
    cv2.imwrite("cv2.imwrite('./experiments/VesselNet/test/sema/8.jpg",cimg_pupil)                                # Save this image
    ...
    cv2.imwrite('./experiments/VesselNet/test/sema/9.jpg',cimg_cat)                            # Display the cataract contour on the original image copy
    ...
    return int(cataract_percentage)
